...
**Core philosophy:** distinguish _what app_ from _what content_ from _how intentional the usage was_ — and do it in an architecture that scales from "just me, one laptop, self-hosted" to "me, multiple devices, one account" to "multiple users on a hosted backend."
...
- This is also what makes multi-tenancy (multiple _users_, not just multiple devices) close to free — the same `user_id` scoping that separates your two devices is what separates two different people's data.
